...
QtLockedFile::~QtLockedFile()
{
    ...
    if(m_mutex_hnd!=0)
    {
        ...
        if(ret == 0)
        {
            qDebug("QtLockedFile::~QtLockedFile(): CloseHandle (mutex): %s",
    ...
    if (m_semaphore_hnd != 0) {
        ...
        if (ret == 0) {
            qDebug("QtLockedFile::~QtLockedFile(): CloseHandle (semaphore): %s",
                        errorCodeToString(GetLastError()).toLatin1().constData());
        }
        m_semaphore_hnd = 0;
    }
...
bool QtLockedFile::lock(QtLockedFile::LockMode mode, bool block)
{
    if(!isOpen())
    {
        qDebug("QtLockedFile::lock(): file is not opened;");
        return false;
    }
    if(mode ==NoLock)
    {
    ...
    if(m_semaphore_hnd==0)
    {
        ...
        if(m_semaphore_hnd==0)
        {
            qDebug("QtLockedFile::lock(): CreateSemaphore: %s",
                   errorCodeToString(GetLastError()).toLatin1().constData());
            return false;
        }
    }
    ...
    else
    {
        ...
        if(m_mutex_hnd==0)
        {
            ...
            if(m_mutex_hnd==0)
            {
                qDebug("QtLockedFile::lock(): CreateMutex: %s",
                       errorCodeToString(GetLastError()).toLatin1().constData());
                return false;
            }
        }
        ...
        if (res == WAIT_TIMEOUT)
            return false;
        if (res == WAIT_FAILED) {
            qDebug("QtLockedFile::lock(): WaitForSingleObject (mutex): %s",
                     errorCodeToString(GetLastError()).toLatin1().constData());
            return false;
        }
        gotMutex = true;
    ...
    for (int i = 0; i < decrement; ++i) {
        ...
        if (res == WAIT_TIMEOUT) {
            if (i) {
                ...
                if (ReleaseSemaphore(m_semaphore_hnd, i, NULL) == 0) {
                    qDebug("QtLockedFile::unlock(): ReleaseSemaphore: %s",
                             errorCodeToString(GetLastError()).toLatin1().constData());
                    // Fall through
                }
            }
        ...
        if (res != WAIT_OBJECT_0) {
            if (gotMutex)
                ReleaseMutex(m_mutex_hnd);
            qDebug("QtLockedFile::lock(): WaitForSingleObject (semaphore): %s",
                        errorCodeToString(GetLastError()).toLatin1().constData());
            return false;
        }
    }
...
{
    if (!isOpen()) {
        qDebug("QtLockedFile::unlock(): file is not opened");
        return false;
    }
